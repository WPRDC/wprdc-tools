...
from .models import *
from .util import parse_options, geocode_file, forward_geocode, spatial_query
...
def reverse_geocode(request):
    ...
    if request.method == 'GET':
        ...
        if pnt:
            ...
            if pin is None:
                try:
                    parcels = Parcel.objects.filter(geom__contains=pnt)
                    assert len(parcels) == 1
                    results['pin'] = parcels[0].pin
                except AssertionError:
                    results['error'] = 'Unable to find PIN because len(parcels) == {} and parcels == {}'.format(
                        len(parcels), parcels)
                except:
                    ...
                    msg += '|| len(parcels) == ' + str(len(parcels))
                    results['error'] = msg
            status = 200
            if not results.keys():
                response = {**BASE_RESPONSE, **
...
def spatial_query_object(request, region_type="", region_name=""):
    ...
    if method not in SPATIAL_QUERIES:
        return JsonResponse(
            {
                **BASE_RESPONSE,
                **{'help': '{} is not a suppoorted method. Use one of the following `{}`'.format(method, '`,`'.join(
                    SPATIAL_QUERIES))}
            }, status=400)
